 ...
 *
 * Generates a branded HTML email for OTP verification.
 ...
 * @param {'email'|'phone'} opts.channel - Which channel this OTP is for
 * @param {number} [opts.expiryMinutes=10] - OTP validity window
 * @returns {string} Complete HTML email string
 */
function buildOtpEmailHtml({ firstName = 'there', otp, channel = 'email', expiryMinutes = 10 }) {
...
<table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;background-color:#ffffff;border-radius:4px;overflow:hidden;box-shadow:0 8px 48px rgba(0,0,0,0.14),0 2px 12px rgba(0,0,0,0.08);">
  ...
  <tr><td style="padding:36px 44px 32px;">
    ...
    <p style="font-size:15px;line-height:1.72;color:#3a3a3a;margin:0 0 20px;font-family:'Georgia',serif;">
      Please use the verification code below to confirm your <strong style="color:#1F3A2D;">${channelLabel}</strong>. This code is valid for <strong style="color:#1F3A2D;">${expiryMinutes} minutes</strong>.
    </p>

    <!-- OTP Digits -->
    <table cellpadding="0" cellspacing="0" style="margin:0 auto 24px;border-collapse:separate;">
    <tr>
      ${digits.map(d => `<td style="width:48px;height:56px;text-align:center;font-size:28px;font-weight:700;font-family:'Courier New',monospace;color:#1F3A2D;background:#F6F4EF;border:2px solid #D8B56A;border-radius:8px;margin:0 4px;letter-spacing:0;">${d}</td>`).join('<td style="width:8px;"></td>')}
    </tr>
    </table>
    ...
    <tr><td style="padding:16px 20px;">
      ...
      <p style="font-size:13px;line-height:1.65;color:#444;margin:0;font-family:'Georgia',serif;">
        Never share this code with anyone. Viramah staff will <strong>never</strong> ask for your OTP. If you did not request this code, please ignore this email.
      </p>
    </td></tr>
    </table>
  ...
  <tr><td style="background-color:#1F3A2D;padding:28px 44px 36px;text-align:center;">
    ...
    <p style="font-size:12px;margin:0 0 18px;font-family:'Courier New',monospace;">
      ...
      <span style="color:#D8B56A;padding:0 6px;">·</span>
    ...
    <p style="font-size:10px;color:rgba(246,244,239,0.28);font-family:'Courier New',monospace;line-height:1.6;margin:0;">
      This is a transactional verification email from Viramah Stay — not a marketing message.
    </p>
  </td></tr>
