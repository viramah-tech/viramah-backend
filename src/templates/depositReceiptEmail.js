 ...
 * @param {number} opts.depositAmount
 * @param {number} opts.registrationFee
 * @param {number} opts.totalPaid
 * @param {string} opts.paymentMode - 'full' | 'half' | 'deposit'
 * @param {string} opts.approvalDate
 * @returns {string} HTML
 */
function buildDepositReceiptEmailHtml({ firstName = 'there', userId, depositAmount, registrationFee, totalPaid, paymentMode, approvalDate }) {
  return `<!DOCTYPE html>
<html lang="en">
...
<table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;background-color:#ffffff;border-radius:4px;overflow:hidden;box-shadow:0 8px 48px rgba(0,0,0,0.14),0 2px 12px rgba(0,0,0,0.08);">
  ...
  <tr><td style="padding:36px 44px 32px;">
    ...
    <p style="font-size:15px;line-height:1.72;color:#3a3a3a;margin:0 0 20px;font-family:'Georgia',serif;">
      Your <strong style="color:#1F3A2D;">deposit of Rs. ${totalPaid.toLocaleString('en-IN')}</strong> has been confirmed by us. Your room is now reserved.
    </p>
    ...
    <tr><td style="padding:20px 24px;">
      <p style="font-size:9px;text-transform:uppercase;letter-spacing:0.38em;font-family:'Courier New',monospace;font-weight:700;color:#D8B56A;margin:0 0 14px;">DEPOSIT SUMMARY</p>
      <table cellpadding="0" cellspacing="0" style="width:100%;">
        <tr>
          <td style="font-size:13px;color:#666;font-family:'Courier New',monospace;padding:4px 0;width:160px;">User ID</td>
          <td style="font-size:14px;color:#1F3A2D;font-weight:700;font-family:'Courier New',monospace;padding:4px 0;">${userId}</td>
        </tr>
        <tr>
          <td style="font-size:13px;color:#666;font-family:'Courier New',monospace;padding:4px 0;">Security Deposit</td>
          <td style="font-size:14px;color:#1F3A2D;font-weight:700;font-family:'Courier New',monospace;padding:4px 0;">Rs. ${depositAmount.toLocaleString('en-IN')}</td>
        </tr>
        <tr>
          <td style="font-size:13px;color:#666;font-family:'Courier New',monospace;padding:4px 0;">Registration Fee</td>
          <td style="font-size:14px;color:#1F3A2D;font-weight:700;font-family:'Courier New',monospace;padding:4px 0;">Rs. ${registrationFee.toLocaleString('en-IN')}</td>
        </tr>
        <tr>
          <td style="font-size:13px;color:#666;font-family:'Courier New',monospace;padding:6px 0 4px;border-top:1px solid #ddd;">Total Paid</td>
          <td style="font-size:16px;color:#1F3A2D;font-weight:700;font-family:'Courier New',monospace;padding:6px 0 4px;border-top:1px solid #ddd;">Rs. ${totalPaid.toLocaleString('en-IN')}</td>
        </tr>
        <tr>
          <td style="font-size:13px;color:#666;font-family:'Courier New',monospace;padding:4px 0;">Payment Mode</td>
          <td style="font-size:14px;color:#1F3A2D;font-weight:700;font-family:'Courier New',monospace;padding:4px 0;">${paymentMode === 'full' ? 'Full Payment' : paymentMode === 'half' ? 'Split Payment' : 'Deposit Only'}</td>
        </tr>
        <tr>
          <td style="font-size:13px;color:#666;font-family:'Courier New',monospace;padding:4px 0;">Approval Date</td>
          <td style="font-size:14px;color:#1F3A2D;font-weight:700;font-family:'Courier New',monospace;padding:4px 0;">${approvalDate}</td>
        </tr>
      </table>
    </td></tr>
    </table>
    ...
    <!-- Next steps -->
    <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#E8F5E9;border-left:4px solid #1F3A2D;border-radius:2px;margin:0 0 24px;">
    <tr><td style="padding:14px 20px;">
      <p style="font-size:9px;text-transform:uppercase;letter-spacing:0.38em;font-family:'Courier New',monospace;font-weight:700;color:#1F3A2D;margin:0 0 8px;">NEXT STEP</p>
      <p style="font-size:13px;line-height:1.65;color:#444;margin:0;font-family:'Georgia',serif;">
        Please visit our office at <strong>Krishna Valley, Vrindavan</strong> for document verification. Carry your original ID proof and a printed copy of this receipt.
      </p>
    </td></tr>
    </table>

    <p style="font-size:13px;line-height:1.65;color:#666;margin:0 0 24px;font-family:'Georgia',serif;">
      A detailed deposit receipt is attached as a PDF with this email for your records.
    </p>
...
module.exports = { buildDepositReceiptEmailHtml };
